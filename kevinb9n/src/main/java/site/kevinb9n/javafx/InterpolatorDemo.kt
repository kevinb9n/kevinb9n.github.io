...
class InterpolatorDemo : Application() {
  ...
  interface Interpolation {
    fun forPoints(points: List<Point2D>): (Double) -> Double
  }

  object JoshInterpolation : Interpolation {
    override fun forPoints(points: List<Point2D>): (Double) -> Double {
  ...
  object JavafxSplineInterpolation : Interpolation {
    fun byEagerness(eagerness: Double): (Double) -> Double {
      return forPoints(listOf(Point2D(0, 0), Point2D(1 - eagerness, eagerness), Point2D(1, 1)))
    }

    override fun forPoints(points: List<Point2D>): (Double) -> Double {
      require(points.size == 3)
      val sorted = points.sortedBy(Point2D::x)
      ...
      fun normalize(p: Point2D) = Point2D(xToUnit.transform(p.x), yToUnit.transform(p.y))

      require(normalize(points[0]) == Point2D(0.0, 0.0))
      require(normalize(points[2]) == Point2D(1.0, 1.0))
  ...
  fun replacePoints(circles: List<Circle>, powerline: Polyline, cubicline: Polyline, splineline: Polyline) {
    val pts = circles.map { Point2D(it.boundsInParent.centerX, it.boundsInParent.centerY) }
